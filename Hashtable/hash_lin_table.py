class HashTableLinPr:                                                   # linear probing
	...
	def insert(self, key, line):
		...
		n = 0
		placed = False
		while self.table[(hashval + n) % self.cap] != None:
			if self.table[(hashval + n) % self.cap][0] == key:			# duplicate
				if line not in self.table[(hashval + n) % self.cap][1]:	# check to see if duplicate line
					self.table[(hashval + n) % self.cap][1].append(line)
				placed = True
				break
			else:
				n += 1
		if placed == False:												# not a duplicate
			if type(line) != list:										# make val into array if not already (rehash passes array to this function)
				line = [line]
			self.table[(hashval + n) % self.cap] = (key, line)
			self.count += 1
		if self.get_load_fact() > 0.75:									# rehash
			newhash = HashTableLinPr(self.cap*2+1)
	...
	def find(self, key):
		...
		found = False
		while self.table[(hashval + n) % self.cap] != None:
			if self.table[(hashval + n) % self.cap][0] == key:
				found = True
				break
			else:
				n += 1
